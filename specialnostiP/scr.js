...
function getCode() {
    ...
    groups.forEach(el => {
        res += `<div>
       <b>${strings[el[0]["Тип"]]}</b>
       <div>${renderSpecialities(el)}</div>
</div>`
    })
    ...
    function renderSpecialities(arr) {
        ...
        return arr.map(el => {
            return `<div>начало обучения:${getDates(el)} - ${el["Специальность переподготовки"]}</div>`
        }).join("")
    }
    ...
    document.querySelector("#iddd").innerHTML = res
...
function newDate(v) {
    let dateStartMS = 25569
    return (+v > 40000) ? 1000*60*60*(24*(+v - dateStartMS)) : v
}

console.log(newDate("идет набор группы"))
console.log(newDate("январь 2023"))
console.log(newDate("24.10.2022"))
console.log(newDate("44817"))
console.log(newDate(""))


function formatDate1(date) {

    let d = (+date > 1000000000) ? new Date(+date) : new Date(Date.parse(date))
    let day = d.getDate() + 1
    let month = ((d.getMonth() + 1) <=9 ) ? "0"+(d.getMonth() + 1) : d.getMonth() + 1
    let year = d.getFullYear()
...
console.log(formatDate1("1662940800000"))
